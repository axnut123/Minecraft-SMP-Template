...
const shop = (Server) => {
  ...
  ShopForm.selectItem = async (player, items, page = 1, sort = 1, search = true) => {
    ...
    for (let i = 0; i < 45; i++) {
      ...
      selectItemForm.button(i, itemData.displayname, itemDesc, itemData.icon, itemData.amount, enchantedItem)
    }
    ...
    if (!res.canceled) {
      if (res.selection == 45) {
        ...
        let confirm = await ShopForm.confirmBuy(player, selectedItem, amount, price)
        ...
        separateAmount(selectedItem.data.amount * amount, item.maxAmount).forEach((amount) => {
          item.amount = amount
  ...
  ShopForm.inputAmount = async (player, item) => {
    const inputAmountForm = new ui.ModalFormData()
      ...
      .textField(`Input amount (Max ${Math.floor(itemCanGet(player, item.item) / item.data.amount)}):`, "Input number here", {defaultValue: "1"})
   ...
   * @returns {boolean}
   */
  ShopForm.confirmBuy = async (player, item, amount, price) => {
    let confirmBuyForm = new ui.MessageFormData()
      .title({ translate: "mce.ui.purchaseconfirmation.title" })
      .body({ translate: "mce.ui.purchaseconfirmation_withamount.description", with: [item.data.displayname, Utility.formatMoney(price), `${item.data.amount * amount}`] })

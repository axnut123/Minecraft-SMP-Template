...
import { PermissionValueType } from "../../Modules/Ranks"
...
const editranks = (Server) => {
  ...
  const LandPermission = (player, rankId, rankData) => {
    ...
    for (let p of Object.keys(permissionList)) {
      ...
      const [description, valueType] = permissionList[p]
      ...
      switch (valueType) {
        case PermissionValueType.boolean:
          ...
          AdminPermissionUI
            .divider()
            .dropdown(description, ["Allow", "No action (Default)", "Deny"], { defaultValueIndex: defaultIndex })
          break

        case PermissionValueType.number:
          AdminPermissionUI
            .divider()
            .textField(description, "Input number here", { defaultValue: `${currentPermission ?? ""}` })
          break

        case PermissionValueType.string:
          AdminPermissionUI
            .divider()
            .textField(description, "Input text here", { defaultValue: `${currentPermission ?? ""}` })
          break
      }
    }
    ...
    ForceOpen(player, AdminPermissionUI).then(res => {
      if (!res.canceled) {
        ...
        for (let i = 0; i < res.formValues.length; i++) {
          const [permissionName, [description, valueType]] = Object.entries(permissionList)[i]
          let permissionValue = res.formValues[i]

          // console.warn(permissionName, permissionValue)
          if (rankId == Server.Ranks.DefaultRankTag) {
            if (typeof permissionValue == "number" && permissionValue == 1)
              continue

            if (typeof permissionValue == "string" && permissionValue.trim() == "")
              continue
          }

          if (valueType == PermissionValueType.boolean) {
          ...
          } else if (valueType == PermissionValueType.number) {
            if (permissionValue.trim() == "") {
              delete rankData.landPermissions[permissionName]
            } else {
              if (!Number.isSafeInteger(Number(permissionValue))) continue
              rankData.landPermissions[permissionName] = Number(permissionValue)
            }
          } else if (valueType == PermissionValueType.string) {
            if (permissionValue.trim() == "") {
              delete rankData.landPermissions[permissionName]
            } else {
              rankData.landPermissions[permissionName] = permissionValue
            }
          }
        }

...
export class Database {
    #onSetCallback; #onDeleteCallback
    /**
     * @param {String} name 
     */
    constructor(name) {
    ...
    delete(key) {
        ...
        this.#onDeleteCallback.forEach(callback => callback({ key }))
    }
    ...
    clear() {
        ...
        this.Database.clear()
        keys.forEach((key) => this.#onDeleteCallback.forEach(callback => callback({ key })))
    }
    ...
    onSet = {
        /**
         * Fires when database got setted
    ...
    onDelete = {
         ...
         */
        subscribe: (callback) => {
            this.#onDeleteCallback.push(callback)
            return { callback }
        },
        unsubscribe: (listener) => {
            const index = this.#onDeleteCallback.indexOf(listener.callback);
            if (index !== -1) {
                this.#onDeleteCallback.splice(index, 1);
            }
        }
    }
}
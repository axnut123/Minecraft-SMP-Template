...
class ItemDatabase {
  ...
  async #waitLoaded() {
    return new Promise((resolve) => {
      let systemId = system.runInterval(() => {
        const ticking = world.tickingAreaManager.getTickingArea("PaoDatabase")
        if (ticking?.isFullyLoaded) {
          system.clearRun(systemId)
          resolve()
        }
      }, 10)
  ...
  async #init() {
    if (!world.tickingAreaManager.hasTickingArea("PaoDatabase")) {
      const size = 2
      await world.tickingAreaManager.createTickingArea("PaoDatabase", {
        dimension: overworld,
